...
public class HUDUtils : MonoBehaviour
{
    // References
    ...
    {
        // References
        mouse = GameObject.Find("Player").GetComponent<MouseController>();
    ...
    void Update()
    {
        ...
        // Crosshair
        RaycastHit hit;
        if(Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, mouse.GetSliceDistance(), 1 << 6)) {
            ...
            Physics.SphereCast(Camera.main.transform.position, mouse.GetAimAssistSize(),
    ...
    public void IncrementScrap(int amount) {
        if(scrap < scrapCap && !GameManager.Instance.frozen){
            ...
            scrapAlarmed = false;
            mouse.PlaySound(pickup);
        } else if(!scrapAlarmed) {
            scrapAlarmed = true;
            mouse.PlaySound(reject);
        }
    }
